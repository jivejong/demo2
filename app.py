...
env_val = os.environ.get("GENAI_API_KEY")
st.write(f"Detected in OS Env? {'✅ Yes' if env_val else '❌ No'}")

# 3. Validation Logic
if "GENAI_API_KEY" in st.secrets:
    st.success("The key is officially active!")
else:
    st.error("The key is missing from the container.")
    st.info("💡 Action: Ensure there are no spaces/comments in the Secrets TOML box.")

# Check if the key is in the secrets dictionary
if "GENAI_API_KEY" in st.secrets:
    api_key = st.secrets["GENAI_API_KEY"]
    st.write("✅ Key detected in Streamlit Secrets.")
else:
    st.error("❌ Key NOT detected in Streamlit Secrets.")
    # Show what IS detected to help debug (e.g., 'user', 'password')
    st.write("Found keys:", list(st.secrets.keys()))
...
# Instead of relying on the environment, pull directly from the secret object
api_key = st.secrets["GENAI_API_KEY"]
